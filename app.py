...
if keyword and search_clicked:
    ...
    try:
        ...
        with st.spinner("Generating enhanced cross-platform synthesis..."):
            ...
            # --- BLEU SYNTHESIS FIDELITY ---
            bleu_score = metrics.compute_bleu_score(
                synthesis['emerging_insight'],
                [r_data['summary'], t_data['summary']]
            )

            if bleu_score is not None:
                if bleu_score >= 0.25:
                    fidelity_label = "High Fidelity"
                    fidelity_color = "#2ecc71"
                elif bleu_score >= 0.10:
                    fidelity_label = "Moderate Fidelity"
                    fidelity_color = "#f39c12"
                else:
                    fidelity_label = "Low Fidelity (generic language)"
                    fidelity_color = "#e74c3c"

                st.markdown(f"""
                <div class="metric-card" style="border-left: 4px solid {fidelity_color}; margin-top: 0.5rem;">
                    <h4 style="margin: 0 0 0.3rem 0;">Synthesis Fidelity (BLEU)</h4>
                    <p style="margin: 0; font-size: 0.85rem; opacity: 0.7;">
                        Measures how well the Emerging Insight preserves key vocabulary
                        from both the Reddit and Twitter summaries it was built from.
                        Higher = synthesis stayed grounded in platform-specific language.
                    </p>
                    <p style="margin: 0.6rem 0 0 0; font-size: 1.4rem; font-weight: 700; color: {fidelity_color};">
                        {bleu_score:.4f}
                        <span style="font-size: 0.85rem; font-weight: 400; margin-left: 0.5rem;">
                            — {fidelity_label}
                        </span>
                    </p>
                </div>
                ...
                st.info("Synthesis Fidelity (BLEU): Unavailable — install nltk (`pip install nltk`) to enable.")
